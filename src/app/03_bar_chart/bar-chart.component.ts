...
export class BarChartComponent implements OnInit {
  ...
  private initAxis() {
    ...
    this.y = d3Scale
      .scaleLinear()
      .domain([0, d3Array.max(this.data, (d: any) => d[this.chartMeasure.dimension2] + 20)])
      .range([this.height - 100, 0]);
    this.y2 = d3Scale
      .scaleLinear()
      .domain([0, d3Array.max(this.data, (d: any) => d[this.chartMeasure.dimension2])])
      .range([this.height2, 0]);
    this.brush = d3Brush.brushX().extent([
      [0, 0],
      [this.width, this.height2]
    ]).on('brush', this.brushed.bind(this));
    this.zoom = d3Zoom.zoom().scaleExtent([1, Infinity]).translateExtent([
      [0, 0],
  ...
  private drawChart() {
    ...
    this.xAxis = d3Axis.axisBottom(this.x).tickSizeInner(0).tickSizeOuter(0);
    this.xAxis2 = d3Axis.axisBottom(this.x2).tickSizeInner(0).tickSizeOuter(0);
    ...
    let rects = focus_group.selectAll('rect')
      .data(this.data);
    ...
    newRects1.append('text')
      .text((d: any) => { return d[this.chartMeasure.dimension2]; })
      .attr('text-anchor', 'middle')
      .attr('x', (d: any) => this.x(d[this.chartMeasure.dimension1]))
    ...
    newRects1.append('image')
      ...
      .attr('id', (d: { StaffNumber: string; }) => { return d.StaffNumber + 'pattern'; })

    // circle on image - added
    newRects1.append('circle')
      .attr('class', 'cir')
      .attr('r', 16)
      // .attr('cy', 16)
      // .attr('cx', 16)
      ...
      .style('fill', (d: { StaffNumber: string; }) => { return 'url(#' + d.StaffNumber + 'pattern)'; });

    focus_group = this.context.append('g');
    focus_group.attr('clip-path', 'url(#clip)');

    const brushRects = focus_group.selectAll('rect')
      .data(this.data);
    ...
    this.context.append('g')
      ...
      .call(this.xAxis2).selectAll('text').remove();
  ...
  private brushed(event: {
    ...
    this.x.domain(nD);
    this.focus.selectAll('.bottomBars')
      .style('opacity', (d: any) => this.x.domain().indexOf(d[this.chartMeasure.dimension1]) === -1 ? 0 : 100)
      .attr('x', (d: any) => {
        let xValue: any;
        if (this.x(d[this.chartMeasure.dimension1]) + this.x.bandwidth() / 2 - 30) {
          xValue = this.x(d[this.chartMeasure.dimension1]) + this.x.bandwidth() / 2 - 30;
        }
        return xValue;
      })
      .attr('y', (d: any) => {
      ...
      .attr('opacity', 0.85)
      .attr('width', this.x.bandwidth());
    this.focus.select('.x.axis').call(this.xAxis).selectAll('text').remove();

    this.focus.selectAll('.img')
      .attr('xlink:href', (d: any) => { return d.imageUrl; })
      .style('opacity', (d: any) => this.x.domain().indexOf(d[this.chartMeasure.dimension1]) === -1 ? 0 : 100)
      .attr('width', 30)
      .attr('x', (d: any) => {
  ...
  private clicked(event: any) {
    ...
    this.tooltip
      ...
      .text(`${x}`);
    this.tooltip.select('text.value_feature')
      .text(`${y}`);
    this.tooltip
